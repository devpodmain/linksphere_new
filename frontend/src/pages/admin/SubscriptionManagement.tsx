...
const SubscriptionManagement: React.FC = () => {
  const [users, setUsers] = useState<AdminSubscription[]>([]);
  ...
  const [manageModalUser, setManageModalUser] = useState<AdminSubscription | null>(null);
  ...
  const [historyModalUser, setHistoryModalUser] = useState<AdminSubscription | null>(null);
  ...
  const fetchUsers = useCallback(
    async (page = 1) => {
      setLoading(true);
      try {
        const params: AdminSubscriptionListParams = {
          page,
        ...
        const response = await fetchAdminSubscriptions(params);
  ...
  const openManageModal = (user: AdminSubscription) => {
    const defaultPeriod =
      user.subscription_plan === 'free'
        ? ''
        : (user.subscription_period as SubscriptionPeriod | null) ??
          (PLAN_PERIOD_OPTIONS[user.subscription_plan][0] ?? '');

    const expiresInput = user.subscription_plan === 'free'
      ? ''
  ...
  const openHistoryModal = async (user: AdminSubscription) => {
    setHistoryModalUser(user);
    setHistoryEntries(user.history ?? []);
    setHistoryLoading(true);
    try {
      const { data } = await fetchAdminSubscriptionHistory(user.id);
      if (data.success) {
  ...
  const renderEmptyState = () => (
    <tbody>
      <tr>
        <td colSpan={8} className="px-4 py-10 text-center text-secondary-500">
          No users found.
        </td>
      </tr>
    </tbody>
        ...
        <table className="min-w-full divide-y divide-secondary-200">
          ...
          {loading ? (
            ...
            <tbody className="bg-white divide-y divide-secondary-100">
              {users.map((user) => (
                <tr key={user.id}>
                  <td className="px-4 py-3 text-sm font-semibold text-secondary-900">{user.name}</td>
      ...
      {historyModalUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 px-4">
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6">
            ...
            {historyLoading ? (
              <p className="text-sm text-secondary-500">Loading history...</p>
            ) : historyEntries.length === 0 ? (
              <p className="text-sm text-secondary-500">No subscription history recorded.</p>
            ) : (
              <div className="space-y-3 max-h-80 overflow-y-auto pr-2">
                {historyEntries.map((entry, index) => (
                  <div key={`${entry.changed_at}-${index}`} className="border border-secondary-200 rounded-lg p-3">
                    <p className="text-sm font-medium text-secondary-900">{formatHistoryLine(entry)}</p>
                    <p className="text-xs text-secondary-500 mt-1">
                      Changed {formatDisplayDate(entry.changed_at)}
